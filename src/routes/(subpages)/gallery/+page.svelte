	...
	{#each gallery_items as item, i}
		<section class="w-full {i % 2 === 0 ? 'bg-slate-800' : 'bg-slate-900'} text-white p-6">
			<div class="w-full max-w-6xl mx-auto flex justify-center flex-wrap-reverse">
				<div class="flex flex-wrap w-full {item.work_videos.length > 1 ? 'w-full' : 'md:w-2/3'}">
					{#each item.work_videos as youtubeVideoId, j}
						<div
							class=" drop-shadow-sm hover:scale-105 transition-all duration-700 w-full p-2 {item
								.work_videos.length > 1 &&
							(j != item.work_videos.length - 1 || item.work_videos.length % 2 === 0)
								? 'md:w-1/2'
								: ''} {j === item.work_videos.length - 1 &&
							item.work_videos.length > 1 &&
							item.work_videos.length % 2
								? 'md:w-2/3 mx-auto'
								: ' '}"
						>
							<YouTubeIFrame initialVideoID={youtubeVideoId.video_id} playerID="player-{j}-{i}" />
						</div>
					{/each}
				</div>
				<article
					class="w-full p-2 md:pl-6 text-center {item.work_videos.length > 1
						? 'md:w-full basis-100'
				...
				>
					...
					<div
						class="flex flex-wrap justify-center w-full gap-2 {item.work_videos.length > 1
							? 'md:justify-center'
							: 'md:justify-normal'}"
					>
						{#if item.work_roles}
							{#each item.work_roles as role}
								<span class="bg-orange-500 px-2 rounded drop-shadow-lg">
									{role.role.production_role}
								</span>
							{/each}
						{/if}
					</div>
