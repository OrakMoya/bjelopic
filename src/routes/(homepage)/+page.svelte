...
<script>
	...
	import { expoIn, expoInOut, expoOut, linear, quadIn, quadOut, quintIn, quintOut } from 'svelte/easing';
	import { fade, fly } from 'svelte/transition';
	import { onMount } from 'svelte';
	import { tweened } from 'svelte/motion';
	const videoscreensizepercent = 70;
	const bar_size_percent = (100 - videoscreensizepercent) / 2; 
	const title_delay = 1200;
	const title_anim_duration = 5000;

	console.log(videoscreensizepercent);
	console.log(bar_size_percent);
...
<div class="bg-black h-screen w-full flex items-center transition-all">
	<section class="w-full flex flex-wrap items-center justify-center" style="height: {videoscreensizepercent}vh;">
