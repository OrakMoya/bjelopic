...
<SixteenByNineContainer>
	...
	<a href={url}>
		...
		>
			...
			{:else}
				{#if thumbnail_path}
					<img src="/images/gallery/{thumbnail_path}" alt={alt_text} transition:fade class="w-full h-full object-cover absolute top-0" />
				{:else}
					<img src="/images/generic_thumbnail.png" alt={alt_text} transition:fade class="w-full h-full object-cover absolute top-0" />
				{/if}
			{/if}
		</div>
	</a>
